    ...
    bool notify)
{
    ...
    if (__gpio_get_value(me->acc_det_gpio) == me->acc_det_gpio_assert) {
        present = 1;
    } else {
        present = 0;
    }
...
static void max77696_eh_det_work (struct work_struct *work)
{
    ...
    dev_vdbg(me->dev, "CHGINB_STAT %02X\n", me->irq_status);
    __unlock(me);
}
...
static __devinit int max77696_eh_probe (struct platform_device *pdev)
{
    ...
    dev_dbg(me->dev, "Detection interval             %u msec\n",
    ...
    dev_dbg(me->dev, "Accessory detection debouncing %u msec\n",
        pdata->acc_det_debounce_ms);
    dev_dbg(me->dev, "Initial mode                   %s\n",
        (pdata->initial_mode == MAX77696_EH_MODE_SHUTDOWN  )? "SHUTDOWN" :
        (pdata->initial_mode == MAX77696_EH_MODE_CHARGER   )? "CHARGER" :
        (pdata->initial_mode == MAX77696_EH_MODE_ACCESSORY )? "ACCESSORY" :
        (pdata->initial_mode == MAX77696_EH_MODE_AUTODETECT)? "AUTODETECT" :
        "(invalid)");
    dev_dbg(me->dev, "Accessory current limit        %u mA\n",
        pdata->acc_ilimit);

    me->irq                 = max77696_rootint_to_irq(chip, CHGB);
    ...
    if (likely(me->acc_det_gpio > 0)) {
        ...
        rc = request_threaded_irq(__gpio_to_irq(me->acc_det_gpio), NULL,
            max77696_eh_acc_isr,
        ...
        if (unlikely(rc < 0)) {
            dev_err(me->dev, "failed to request IRQ(%d) [%d]\n",
                __gpio_to_irq(me->acc_det_gpio), rc);
            goto out_err_req_acc_irq;
        }

        dev_info(me->dev, "accessory detection: GPIO %u IRQ %u\n",
            me->acc_det_gpio, __gpio_to_irq(me->acc_det_gpio));
...
static __devexit int max77696_eh_remove (struct platform_device *pdev)
{
    ...
    free_irq(me->irq, me);
    free_irq(__gpio_to_irq(me->acc_det_gpio), me);
...
int max77696_eh_set_mode (int mode)
{
    ...
    if (unlikely(!me)) {
        pr_err("%s: max77696_eh is not ready\n", __func__);
        return -ENODEV;
    }
