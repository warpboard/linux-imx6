...
static irqreturn_t max77696_key_up_isr (int irq, void *data)
{
    ...
    for (i = KEY_NKEY-1; i >= 0; i--) {
        max77696_key_report(key, i, KEY_UP_VALUE);
    }
...
static irqreturn_t max77696_key_down_isr (int irq, void *data)
{
    ...
    for (i = 0; i < KEY_NKEY; i++) {
        max77696_key_report(key, i, KEY_DOWN_VALUE);
    }
...
static struct max77696_key_irq_info max77696_key_irq_info[] = {
    #define KEY_IRQ_INFO(_name, _topsys_irq, _isr) \
            {\
                .name       = DRIVER_NAME"."_name,\
                .topsys_irq = MAX77696_TOPSYS_IRQ_##_topsys_irq,\
                .isr        = _isr,\
            }
    KEY_IRQ_INFO("en0.up"  , EN0_RISING , max77696_key_up_isr   ),
    KEY_IRQ_INFO("en0.down", EN0_FALLING, max77696_key_down_isr ),
    KEY_IRQ_INFO("en0.1sec", EN0_1SEC   , max77696_key_1sec_isr ),
    KEY_IRQ_INFO("mrwrn"   , MR_WARNING , max77696_key_mrwrn_isr),
};
