/*
 * MAX77696 32kHz Oscillator Driver
 *
...
{
    static const char *load_cap_str[] = {
        [MAX77696_32K_LOAD_CAP_22PF] = "22pF per node",
        [MAX77696_32K_LOAD_CAP_12PF] = "12pF per node",
        [MAX77696_32K_LOAD_CAP_10PF] = "10pF per node",
        [MAX77696_32K_LOAD_CAP_NONE] = "none"
    };
    static const char *op_mode_str[] = {
        [MAX77696_32K_MODE_LOW_POWER]  = "low power",
        [MAX77696_32K_MODE_LOW_JITTER] = "low jitter"
    };

    struct max77696_chip *chip = dev_get_drvdata(pdev->dev.parent);
    struct max77696_32k_platform_data *pdata = dev_get_platdata(&(pdev->dev));
    ...
    /* Show defaults */
    dev_dbg(me->dev, "xtal driver load cap: %s\n",
        load_cap_str[pdata->load_cap % ARRAY_SIZE(load_cap_str)]);
    dev_dbg(me->dev, "mode of operation:    %s\n",
        op_mode_str[pdata->op_mode % ARRAY_SIZE(op_mode_str)]);
