...
{\
    ...
    rc = max77696_chip_##module_name##_reg_write(chip,\
        max77696_dbg_##module_name##_reg_addr, data);\
    printk(KERN_INFO"write "#module_name" reg addr 0x%02X data 0x%04X rc %d\n",\
    ...
    struct max77696_platform_data *pdata)
{
    ...
    if (likely(pdata->core_supports_debugging_stuff)) {
...
static int (*max77696_chip_reg_read[4]) (struct max77696_chip *chip,
    u8 addr, u16 *data) =
{
    [MAX77696_CORE_MID]  = max77696_chip_pmic_reg_read,
    [MAX77696_RTC_MID]   = max77696_chip_rtc_reg_read,
    [MAX77696_UIC_MID]   = max77696_chip_uic_reg_read,
    [MAX77696_GAUGE_MID] = max77696_chip_gauge_reg_read,
};

static int (*max77696_chip_reg_write[4]) (struct max77696_chip *chip,
    u8 addr, u16 data) =
{
    [MAX77696_CORE_MID]  = max77696_chip_pmic_reg_write,
    [MAX77696_RTC_MID]   = max77696_chip_rtc_reg_write,
    [MAX77696_UIC_MID]   = max77696_chip_uic_reg_write,
    [MAX77696_GAUGE_MID] = max77696_chip_gauge_reg_write,
};

int max77696_chip_read (u8 module, u8 addr, u16 *data)
{
    BUG_ON(!max77696 || module >= ARRAY_SIZE(max77696_chip_reg_read));
...
{
    BUG_ON(!max77696 || module >= ARRAY_SIZE(max77696_chip_reg_write));
