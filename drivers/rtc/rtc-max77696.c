...
#define DRIVER_DESC    "MAX77696 Real Time Clock Driver"
...
#define max77696_rtc_write_irq_mask(me) \
        do {\
            ...
            dev_vdbg((me)->dev, "written RTCINTM 0x%02X [%d]\n", _buf, _rc);\
            if (unlikely(_rc)) {\
    ...
    struct rtc_time *tm)
{
    ...
    dev_vdbg(me->dev, "--> %04d-%02d-%02d %s %02d:%02d:%02d\n",
    ...
    struct rtc_wkalrm *alrm)
{
    ...
    dev_vdbg(me->dev, "read alarm%d:\n", alrm_id);
    dev_vdbg(me->dev,
        "YEAR %02X MONTH %02X DOM %02X HOUR %02X MIN %02X SEC %02X\n",
        buf[RTC_YEAR], buf[RTC_MONTH], buf[RTC_DOM],
        buf[RTC_HOUR], buf[RTC_MIN], buf[RTC_SEC]);
    dev_vdbg(me->dev, "--> %04d-%02d-%02d %02d:%02d:%02d\n",
        tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday,
        tm->tm_hour, tm->tm_min, tm->tm_sec);
    ...
    struct rtc_wkalrm *alrm)
{
    ...
    if (likely(alrm->enabled)) {
        ...
        dev_vdbg(me->dev, "setting alarm%d:\n", alrm_id);
        dev_vdbg(me->dev, "%04d-%02d-%02d %02d:%02d:%02d -->\n",
            tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday,
            tm->tm_hour, tm->tm_min, tm->tm_sec);
        dev_vdbg(me->dev,
...
static __devinit int max77696_rtc_probe (struct platform_device *pdev)
{
    ...
    me->irq = max77696_rootint_to_irq(chip, RTC);
